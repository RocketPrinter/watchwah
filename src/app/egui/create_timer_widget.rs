use chrono::Duration;
use eframe::egui::{Button, ComboBox, Response, Ui};
use tracing::info;
...
pub fn ui(ui: &mut Ui, data: &mut CreateTimerState, state: &State ) {
    ...
    ui.horizontal(|ui| {
        ...
        if data.selected_profile.as_ref().map(|p| state.profiles.contains(p)) == Some(false) {
    ...
    ui.horizontal(|ui|{
        ...
        if ui.selectable_label(matches!(data.selected_goal, TimerGoal::Time(_)), "Time").clicked() {
            data.selected_goal = TimerGoal::Time(Duration::minutes(15));
        }
        if ui.selectable_label(matches!(data.selected_goal, TimerGoal::Todos(_)), "Todo").clicked() {
    ...
    // Start button
    if ui.add_enabled( data.selected_profile.is_some(), Button::new("Start") ).clicked() {
        state.ws_tx.send(ClientToServer::CreateTimer{
